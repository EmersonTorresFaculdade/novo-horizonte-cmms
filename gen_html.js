...
const getOsHtmlV2 = () => {
    const htmlParts = [
        ...
        "'\" class=\"btn\">' + ",
        "($json.body.workOrder && $json.body.workOrder.id) ? 'Acompanhar Minha Ordem' : ($json.body.event === 'password_reset_request' ? 'Redefinir Senha' : 'Acessar Sistema')",
        "'</a></div></div><div class=\"footer\">Novo Horizonte CMMS • Tecnologia em Gestão<br>© 2026 Novo Horizonte</div></div></body></html>'",
    ];
    return "={{ " + htmlParts.join(' + ') + " }}";
}

console.log(getOsHtmlV2());
